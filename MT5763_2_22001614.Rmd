...
tournament_stats <- function(n, rates = win_rates) {
...
```{r}
table_n_matches <- tournament_stats(n = 10000)
```


```{r}
# Generating a bootstrap function 

bootstrap_nmatches <- function(n_straps, store){
    avg_matches <- matrix(nrow = 9, ncol = n_straps)

    for (j in 1 : 9){

      #bootstrap and calculate the mean of each bootstrap sample
      avg_matches[j,] <- unlist(mclapply(1 : n_straps, function(i){
    ...
    substore <- rep(NA, 9)
    for (i in 1 : 9) {
    substore[i] <- median(avg_matches[i,])  
    }

    output <- data.frame(Prob = seq(from = 0.1, to = 0.90, by = 0.1),
                         games_on_average = substore)
...
results_2.1 <- bootstrap_nmatches(1000, table_n_matches)
...
results_2.1 %>% 
  ggplot(aes(x = Prob, y = games_on_average))+
  geom_point()+
  ylim(c(0,9))
