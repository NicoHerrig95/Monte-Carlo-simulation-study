...
#simulations one signle tournament
...
bootstrap_problem_two <- function(n_straps, store){
    ...
    for (j in 1 : 9){ # 9 iterations (as we use 9 win rates)
      ...
      median(store[j, sample(1 : dim(store)[2], size = dim(store)[2], 
                             replace = TRUE)])
      }, mc.cores = 8))
    }
    ...
    for (i in 1 : 9) {
    substore[i] <- median(avg_storer[i,])  
    }
...
```
INTERPRETATION WHEN SEEDS ARE SET \
...
The algorithm below is an altered version of the *tournament_stats_ngames* algorithm. The only difference is that it now uses the third element of the output list of the original underlying algorithm *tournament_sim* (line 406), which is the proportion of matches won in a tournament.
...
store[i,] <- unlist(mclapply(1:n, function(i){
  ...
  }, mc.cores = 8))                           # now true_winrate, as numeric
}
  return(store)
}
```
