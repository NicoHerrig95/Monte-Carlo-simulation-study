...
Using a simple grid search algorithm for seeking the maximum of both $f_1(p)$ and $f_2(p)$, both functions peek at a *p* of `r toString(maximum_func1)` and `r toString(maximum_func2)`, respectively. In conclusion, the likelihood of playing the maximum amount of matches in a tournament decreases with a *p* > 0.78. Therefore, we can observe a negative $\delta$ at *p* = 0.9
