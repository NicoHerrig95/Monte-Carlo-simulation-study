...
\
Lastly, it is of interest how the sample variance of the sampling distribution changes with dependence on the simulations run. This part of the experiment is based on the *law of large numbers* theorem. As an observer, we can assume that with increasing number of simulations (increasing number of random deviates used), our *simulated* $\widehat{Pr}(X>Y)$ approximates the *theoretical* value of $Pr(X>Y)$.\
...
```
The plot above shows that with an increasing number of simulations used, the observed $\widehat{Pr}(X>Y)$ converges to its theoretical value.\
...
\
It now should be taken in consideration how the *number of total matches* changes if we alter the winning rate $p$. As the conditions for ending a tournament are either 7 wins or 3 losses, the number of matches played will be $x \in \mathbb{N} [3,9]$, where x is the number of matches played.
...
As a next step, 10,000 matches per win rate are simulated and stored in a matrix (*table_n_matches*).
```{r warning=FALSE}
rm(.Random.seed)
...
As exact results are of interest, the matrix containing the number of matches per tournament  is put into a non-parametric bootstrap algorithm. The algorithm, using a matrix as an input as well as the number of bootstraps *n* (*per winning rate*), generates the median of the strapped sample in a first step (i.e., *n* medians per fixed winning rate). In a second step, the algorithm then calculates the median of the calculated medians from the bootstrapping in step 1.\
...
This discrepancy is again due to the format of the tournament, i.e. the condition of 7 wins or 3 losses ending the tournament.  Assuming a fixed win rate $p(win) = 0.1$, the probability of loosing three matches in a row is $(1-0.1)^3 = 0.729$, which means a team with a *theoretical* win rate per match of 10% has an *observed* winning per match 0% in 73 out of 100 tournaments. In conclusion, 73% of the time a tournament is played, the difference between the theoretical and the observed win rate is -0.1.\
